...
def get_by_lsst_ss_object_id(ss_object_id: str) -> Optional[Locus]:
    """
    Gets an ANTARES locus by its LSST Solar System Object ID.
    ...
    """
    if not isinstance(ss_object_id, str):
        raise ValueError("`ss_object_id` must be a string")
    try:
        return next(
            search(
                {
                    "query": {
                        "bool": {
                            "filter": {
                                "term": {
                                    "properties.survey.lsst.ss_object_id": ss_object_id
                                },
                            },
                        },
                    },
