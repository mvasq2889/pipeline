...
def test_get_by_lsst_dia_object_id(lsst_mock_api):
    locus = search.get_by_lsst_dia_object_id("169566724088135687")
    assert locus.locus_id == "ANT2025uns34defs98p"


def test_get_by_lsst_dia_object_id_returns_none(lsst_mock_api):
    assert search.get_by_lsst_dia_object_id("-1") is None


def test_get_by_lsst_dia_object_id_using_int(lsst_mock_api):
    with pytest.raises(ValueError):
        search.get_by_lsst_dia_object_id(169566724088135687)
...
def test_get_by_lsst_ss_object_id(lsst_mock_api):
    locus = search.get_by_lsst_ss_object_id("21163641725858384")
    assert locus.locus_id == "ANT2026c7l4z1c07rwn"


def test_get_by_lsst_ss_object_id_returns_none(lsst_mock_api):
    assert search.get_by_lsst_ss_object_id("-1") is None


def test_get_by_lsst_ss_object_id_using_int(lsst_mock_api):
    with pytest.raises(ValueError):
        search.get_by_lsst_ss_object_id(21163641725858384)
...
def test_get_locus_id_none_returns_random_locus_id(mock_api):
    random_locus_id = search.get_random_locus_id()
    assert random_locus_id is not None
    assert isinstance(random_locus_id, str)


def test_get_random_locus_with_n_returns_n_locus_ids(mock_api):
    n_wanted_locus = 2
    locus_ids = search.get_random_locus_ids(n_wanted_locus)
    assert n_wanted_locus == len(locus_ids)


def test_get_random_locus_ids_greater_than_1000_raises_valueerror():
    n_wanted_locus = 1001
    with pytest.raises(ValueError) as err:
        search.get_random_locus_ids(n_wanted_locus)
        assert "n must be less than or equal to 1000" in str(err)


def test_get_random_loci_with_n_retusn_n_loci(mock_api):
    n_wanted_locus = 2
    loci = search.get_random_locus_ids(n_wanted_locus)
    assert n_wanted_locus == len(loci)


def test_get_random_locus_returns_random_locus(mock_api):
    locus = search.get_random_locus()
