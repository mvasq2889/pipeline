...
@pytest.fixture()
def lsst_mock_api(requests_mock):
    with open(
        os.path.join(API_RESPONSE_DIRECTORY, "lsst-loci-ANT2025uns34defs98p.json")
    ) as f:
        response = json.load(f)
        path = 'loci?sort=-properties.newest_alert_observation_time&elasticsearch_query[locus_listing]={"query": {"bool": {"filter": {"term": {"properties.survey.lsst.dia_object_id": "169566724088135687"}}}}}'
        requests_mock.get(config["ANTARES_API_BASE_URL"] + path, json=response)
    with open(
        os.path.join(
            API_RESPONSE_DIRECTORY,
            "lsst-loci-ANT2026c7l4z1c07rwn-ANT2026ryw0tsp2rjgf.json",
        )
    ) as f:
        response = json.load(f)
        path = 'loci?sort=-properties.newest_alert_observation_time&elasticsearch_query[locus_listing]={"query": {"bool": {"filter": {"term": {"properties.survey.lsst.ss_object_id": "21163641725858384"}}}}}'
        requests_mock.get(config["ANTARES_API_BASE_URL"] + path, json=response)
    with open(
        os.path.join(API_RESPONSE_DIRECTORY, "lsst-diaobjectid-loci-empty.json")
    ) as f:
        response = json.load(f)
        path = 'loci?sort=-properties.newest_alert_observation_time&elasticsearch_query[locus_listing]={"query": {"bool": {"filter": {"term": {"properties.survey.lsst.dia_object_id": "-1"}}}}}'
        requests_mock.get(config["ANTARES_API_BASE_URL"] + path, json=response)
    with open(
        os.path.join(API_RESPONSE_DIRECTORY, "lsst-ssobjectid-loci-empty.json")
    ) as f:
        response = json.load(f)
        path = 'loci?sort=-properties.newest_alert_observation_time&elasticsearch_query[locus_listing]={"query": {"bool": {"filter": {"term": {"properties.survey.lsst.ss_object_id": "-1"}}}}}'
        requests_mock.get(config["ANTARES_API_BASE_URL"] + path, json=response)
    with open(
        os.path.join(
            API_RESPONSE_DIRECTORY, "lsst-loci-ANT2025uns34defs98p-alerts.json"
        )
    ) as f:
        response = json.load(f)
        path = "loci/ANT2025uns34defs98p/alerts"
        requests_mock.get(config["ANTARES_API_BASE_URL"] + path, json=response)
